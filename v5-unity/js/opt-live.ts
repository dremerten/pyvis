...
- abstract out components within pycodevis.js to prevent ugly code
...
import { ExecutionVisualizer, assert, brightRed, darkArrowColor, lightArrowColor, SVG_ARROW_POLYGON, htmlspecialchars } from './pycodevis';
...
export class OptLiveFrontend extends OptFrontend {
  ...
  constructor(params) {
    ...
    // TODO: eliminate redundancies with pycodevis.ts
    $("#jmpFirstInstr").click(() => {
  ...
  updateStepLabels() {
    ...
    // TODO: prevent copy and paste with pycodevis.ts
    ...
    // copied from pycodevis.js -- TODO: integrate this code better
      ...
      ruiDiv.find('#raw_input_submit_btn')
        ...
        .click(() => {
          ...
          if (TogetherJS.running) {
            alert("Warning: user inputs do NOT work well in live help/chat mode. We suggest you use the regular Python code-vis visualizer instead.");
          }
          */
    ...
    if (curEntry.event === 'exception' ||
      ...
      if (curEntry.exception_msg == "Unknown error") {
        $("#frontendErrorOutput").html('Unknown error: <a target="_blank" href="https://github.com/pgbovine/OnlinePythoncode-vis/blob/master/unsupported-features.md">read this page for more info</a>');
  ...
  finishSuccessfulExecution() {
    ...
    // set up execution slider, code inspired by pycodevis.js:
    // TODO: eventually unify this code with pycodevis.js to avoid duplication
    ...
    // above, perhaps using renderStep()
    myVisualizer.add_pycodevis_hook(
      "end_updateOutput",
      (args) => {
        /* 2019-04-09: disabled shared sessions in opt-live.ts
