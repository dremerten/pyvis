...
/* colors - see pycodevis.css for more colors */
...
export class ExecutionVisualizer {
  ...
  //
  // dat is data returned by the Python code-vis backend consisting of two fields:
  ...
  /* API for adding a hook, created by 
   ...
   An external user should call
     add_pycodevis_hook("hook_name_here", function(args) {...})
  ...
  add_pycodevis_hook(hook_name, func) {
    if (this.pycodevis_hooks[hook_name])
      this.pycodevis_hooks[hook_name].push(func);
    else
      this.pycodevis_hooks[hook_name] = [func];
  }
  ...
  Although add_pycodevis_hook allows the hooked function to
  ...
  try_hook(hook_name, args) {
    if (this.pycodevis_hooks[hook_name]) {
      for (var i = 0; i < this.pycodevis_hooks[hook_name].length; i++) {
        // apply w/o "this", and pack sole arg into array as required by apply
        var handled_and_result
          = this.pycodevis_hooks[hook_name][i].apply(null, [args]);
  ...
  render() {
    ...
    if (this.params.embeddedMode) {
      ...
      base.append('<div style="font-size: 8pt; margin-bottom: 10px;"><a href="http://pythoncode-vis.com" target="_blank" style="color: #3D58A2;">Python code-vis</a> by <a href="https://twitter.com/pgbovine" target="_blank" style="color: #3D58A2;">Andre Merten</a></div>');
    ...
    // enable left-right draggable pane resizer
    base.resizable({
  ...
  updateOutputFull(smoothTransition) {
    ...
    if (myViz.curLineExceptionMsg) {
      if (myViz.curLineExceptionMsg === "Unknown error") {
        // myViz.navControls.showError('Unknown error: <a target="_blank" href="https://github.com/pgbovine/OnlinePythoncode-vis/blob/master/unsupported-features.md">read this page for more info</a>'); # Removed unsupported feature-list chai@python3
  ...
  // integrated into pycodevis.js by Andre Merten
  ...
  constructor(owner, domRoot, heightOverride = null) {
    ...
    var outputsHTML =
      '<div id="progOutputs">\
        ...
        <textarea id="pyStdout" cols="40" rows="12" wrap="off" readonly></textarea>\
       </div>';
    ...
    var stdoutHeight = '200px';
    // heuristic for very small outputs so we don't end up with a tiny box
  ...
  constructor(owner, domRoot, domRootD3,
    ...
    if (lang !== undefined) {
      if (lang === 'js') {
        ...
        this.domRoot.find('#langDisplayDiv').html('Python 3.14');
      } else if (lang === 'py3anaconda') {
        this.domRoot.find('#langDisplayDiv').html('Python 3.14 with <a target="_blank" href="https://docs.anaconda.com/anaconda/">Anaconda</a> <font color="#e93f34">EXPERIMENTAL!</font><br/>(slower than <a target="_blank" href=\"visualize.html#py=3\">regular Python 3.14</a>, but lets you import more modules)');
      } else if (lang === 'c') {
        if (this.owner.params.embeddedMode) {
          this.domRoot.find('#langDisplayDiv').html('C (gcc 4.8, C11)');
        } else {
          // this.domRoot.find('#langDisplayDiv').html('C (gcc 4.8, C11)<br/><font color="#e93f34">EXPERIMENTAL!</font> <a href="https://github.com/pgbovine/OnlinePythoncode-vis/blob/master/unsupported-features.md" target="_blank">known bugs/limitations</a>');
        ...
        if (this.owner.params.embeddedMode) {
          ...
          // this.domRoot.find('#langDisplayDiv').html('C++ (gcc 4.8, C++11)<br/><font color="#e93f34">EXPERIMENTAL!</font> <a href="https://github.com/pgbovine/OnlinePythoncode-vis/blob/master/unsupported-features.md" target="_blank">known bugs/limitations</a>');
  ...
  renderPyCodeOutput() {
    ...
    var codeOutputD3 = this.domRootD3.select('#pyCodeOutputDiv')
      ...
      .attr('class', function (d, i) {
        ...
        // sessions (kinda leaky abstraction since pycodevis.ts shouldn't
...
class NavigationController {
  ...
  showError(msg: string) {
    if (msg) {
      ...
      // <span style="font-size: 9pt; color: #666">(see <a href="https://github.com/pgbovine/OnlinePythoncode-vis/blob/master/unsupported-features.md" target="_blank">unsupported features</a>)</span>`).show();
    } else {
