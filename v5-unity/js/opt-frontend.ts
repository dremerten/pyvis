...
- we're referring to top-level CSS selectors on the page; maybe use a
  this.domRoot pattern like in pycodevis.ts?
...
import { ExecutionVisualizer, assert, htmlspecialchars } from './pycodevis';
...
export class OptFrontend extends AbstractBaseFrontend {
  ...
  constructor(params = {}) {
    ...
    $('#genEmbedBtn').bind('click', () => {
      ...
      var domain = "http://pythoncode-vis.com/"; // for deployment
  ...
  initAceEditor(height: number) {
    ...
    this.setAceMode();
    this.pyInputAceEditor.setReadOnly(false);
    this.pyInputAceEditor.focus();

    // Allow pasting code even if the Ace hidden textarea loses focus (seen in some browsers / iframes)
    document.addEventListener('paste', (evt: ClipboardEvent) => {
      if (!this.pyInputAceEditor) return;
      ...
      const insideEditor = targetEl ? jQuery(targetEl).closest('#codeInputPane').length > 0 : false;
      const isAceInput = targetEl ? targetEl.classList.contains('ace_text-input') : false;
      if (insideEditor || isAceInput) {
        evt.preventDefault();
        this.pyInputAceEditor.insert(text);
        this.pyInputAceEditor.focus();
      }
    }, true);
  }
