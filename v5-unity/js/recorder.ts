...
/* TODOs:
  ...
  - test by recording locally (with python/js/etc. backends running on
    localhost) and then replaying remotely on pythoncode-vis.com, since
    that's what students will ultimately be doing.
    - also make a special entry in codechella logs for code-visial replays
  ...
  - in playback mode, set a more instructive username for the code-vis's
  ...
  - ideally don't send events to the togetherjs when you're in recording
    ...
    - maybe make a mock websockets interface to FAKE a connection to the
      ...
      both for performance and for being able to ship code-visials as
...
minor-ish:
  ...
  - add title and description fields so that codcasts can serve as
    independent mini-code-visials and WORKED EXAMPLES
  ...
  - to prevent weird crashes from encoding mp3's in JS itself, maybe
    ...
    code-visial recording from the compressing and also give more flexibility
    to the format
