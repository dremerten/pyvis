...
var TogetherJSConfig_siteName = "Online Python code-vis shared session";
var TogetherJSConfig_toolName = "Online Python code-vis shared session";
...
function initTogetherJS() {
  ...
  TogetherJS.hub.on("togetherjs.hello-back", function(msg) {
    ...
    if (msg.name == p.Self.name) {
      ...
      do {
        if (!isNaN(count)) {
          newName = toks[0] + ' ' + String(count + 1); // e.g., "code-vis 3"
          count++;
        }
        else {
